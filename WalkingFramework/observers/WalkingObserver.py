...
class WalkingObserver(LeafSystem):
    ...
    def CalcOutput(self,context: Context,output: AbstractValue = None):
        if output is not None:
            output.set_value(np.zeros(self.n_state))
        return
    
    def AddToBuilderAndConnect(self, builder: DiagramBuilder, robot: WalkingRobot, forced_update=False):
        builder.AddSystem(self)
        builder.Connect(robot.sensing_out, self.sensing_in)
        if forced_update:
            self.DeclarePerStepUnrestrictedUpdateEvent(lambda context, state: self.CalcOutput(context, None))
